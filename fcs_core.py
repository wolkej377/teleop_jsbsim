...
class AircraftSimulation:
    def __init__(self, max_time=1000.0, init_xml="./lyj_init.xml", log_csv="c310_land.csv", visualizer=None, visualizer_sync=False):
        ...
        self.visualizer_sync = visualizer_sync
    ...
    def cruise(self):
        ...
        self.fdm['fcs/advance-cmd-norm[0]'] = 1.0
    ...
    def visualize_sync(self):
        if self.visualizer:
            self.visualizer.add_data(
                # sync=self.visualizer_sync,
                t=self.sim_time,
    ...
    # ---- 仿真循环 ----
    def run_simulation(self, initial_task="cruise"):
        if initial_task == "cruise":
            self.cruise()
...
if __name__ == "__main__":
    from flight_visualizer import ChartVisualizer, CSVVisualizer
    # vis = ChartVisualizer()
    sim = AircraftSimulation(visualizer=None)
    ...
    csv_vis = CSVVisualizer("c310_land.csv")
    csv_vis.plot(
